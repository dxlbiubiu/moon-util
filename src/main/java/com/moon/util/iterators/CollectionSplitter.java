 ...
 * 或者使用
 * {@link com.moon.util.IteratorUtil#split(Collection)}
 * {@link com.moon.util.IteratorUtil#split(Collection, int)}
 * {@link com.moon.util.IteratorUtil#splitter(Collection, Consumer)}
 * {@link com.moon.util.IteratorUtil#splitter(Collection, int, Consumer)}
 *
 * @author benshaoye
 * @date 2018/9/11
 */
public class CollectionSplitter<E, T extends Collection<E>> implements Iterator<T> {
    private final static int DEFAULT_COUNT = 16;
    /**
     * 默认集合类
     */
    ...
    public CollectionSplitter(T t) {
        this(t, DEFAULT_COUNT);
    }

    public CollectionSplitter(T t, int count) {
        this.split(t, count);
    }
     ...
     * @return
     */
    public CollectionSplitter split(T t, int count) {
        if (count < 1) {
